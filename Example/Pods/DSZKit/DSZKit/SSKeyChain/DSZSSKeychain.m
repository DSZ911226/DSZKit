...
NSString *const kDSZSSKeychainErrorDomain = @"com.samsoffes.DSZSSKeychain";

NSString *const kDSZSSKeychainAccountKey = @"acct";
NSString *const kDSZSSKeychainCreatedAtKey = @"cdat";
NSString *const kDSZSSKeychainClassKey = @"labl";
NSString *const kDSZSSKeychainDescriptionKey = @"desc";
NSString *const kDSZSSKeychainLabelKey = @"labl";
NSString *const kDSZSSKeychainLastModifiedKey = @"mdat";
NSString *const kDSZSSKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef DSZSSKeychainAccessibilityType = NULL;
#endif

@interface DSZSSKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation DSZSSKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = DSZSSKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kDSZSSKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (DSZSSKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return DSZSSKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (DSZSSKeychainAccessibilityType) {
		CFRelease(DSZSSKeychainAccessibilityType);
	}
	DSZSSKeychainAccessibilityType = accessibilityType;
}
#endif
