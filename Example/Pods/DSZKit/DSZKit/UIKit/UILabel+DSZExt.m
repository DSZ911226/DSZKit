...
- (id<DSZAttributeTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<DSZAttributeTapActionDelegate>)delegate
{
    objc_setAssociatedObject(self, @selector(delegate), delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

#pragma mark - mainFunction
- (void)DSZ_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (NSString *string , NSRange range , NSInteger index))tapClick
{
    [self DSZ_getRangesWithStrings:strings];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self DSZ_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        
        if (weakSelf.tapBlock) {
            weakSelf.tapBlock (string , range , index);
        }
        
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(DSZ_attributeTapReturnString:range:index:)]) {
            [weakSelf.delegate DSZ_attributeTapReturnString:string range:range index:index];
        }
        
        if (self.isTapEffect) {
            
            [self DSZ_saveEffectDicWithRange:range];
            
            [self DSZ_tapEffectWithStatus:YES];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self DSZ_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL)DSZ_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGAffineTransform transform = [self DSZ_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self DSZ_getLineBounds:line point:linePoint];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if (self.isTapEffect) {
        ...
        [self performSelectorOnMainThread:@selector(DSZ_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
...
- (CGAffineTransform)DSZ_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)DSZ_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
...
- (void)DSZ_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)DSZ_saveEffectDicWithRange:(NSRange)range
{
...
- (void)DSZ_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf DSZ_getStringWithRange:range]];
...
- (NSString *)DSZ_getStringWithRange:(NSRange)range
{
                                      ...
                                      font:(UIFont *)font
{
    ...
    [arrString addAttributes:@{NSFontAttributeName:font,
                       ...
                       range:[wholeString rangeOfString:targetString]];
    return arrString;
}
