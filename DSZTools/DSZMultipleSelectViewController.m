...
@implementation DSZSelectModel
+ (DSZSelectModel *)initWithname:(NSString *)name ID:(NSString *)ID isSelected:(BOOL)isSelected {
    DSZSelectModel *selectM = [DSZSelectModel new];
    selectM.name = name;
    selectM.ID = ID;
...
- (void)rightBarBtnAction {
    if (self.result) {
        ...
        for (DSZSelectModel *model in self.dataSource) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DSZSelectModel *model =self.dataSource[indexPath.row];
