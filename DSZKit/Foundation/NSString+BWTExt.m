...
#import "NSData+BWTExt.h"
#import "NSDate+BWTExt.h"
...
@implementation NSString (BWTExt)
...
+ (NSString *)p_returnDeviceID:(NSString *)deviceID
    ...
    if(device.length == 0) {
        ...
        device = [BWTSSKeychain passwordForService:bundleIdentifier account:bundleIdentifier];

        if(device.length == 0) {
            NSString *UUID = [self stringWithUUID];
            device = [[NSString stringWithFormat:@"%@%@",UUID, bundleIdentifier] md5String];
            [BWTSSKeychain setPassword:device forService:bundleIdentifier account:bundleIdentifier];
        }
...
- (NSString *)aes128Encrypt:(NSString *)key {
    return [BWTAES128 AES128Encrype:self key:key];
}

- (NSString *)aes128Decrypt:(NSString *)key {
    return [BWTAES128 AES128Decrype:self key:key];
}
