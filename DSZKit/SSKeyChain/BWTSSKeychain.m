...
#import "BWTSSKeychain.h"

NSString *const kBWTSSKeychainErrorDomain = @"com.samsoffes.BWTSSKeychain";

NSString *const kBWTSSKeychainAccountKey = @"acct";
NSString *const kBWTSSKeychainCreatedAtKey = @"cdat";
NSString *const kBWTSSKeychainClassKey = @"labl";
NSString *const kBWTSSKeychainDescriptionKey = @"desc";
NSString *const kBWTSSKeychainLabelKey = @"labl";
NSString *const kBWTSSKeychainLastModifiedKey = @"mdat";
NSString *const kBWTSSKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef BWTSSKeychainAccessibilityType = NULL;
#endif

@interface BWTSSKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation BWTSSKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = BWTSSKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kBWTSSKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (BWTSSKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return BWTSSKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (BWTSSKeychainAccessibilityType) {
		CFRelease(BWTSSKeychainAccessibilityType);
	}
	BWTSSKeychainAccessibilityType = accessibilityType;
}
#endif
